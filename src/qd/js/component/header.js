        ...
        initAutocompleteMod: function () {
                ...
                onSearchComplete: (function (query, data, resultContainer) {
                    if (data && data.length != 0) {
                        $(resultContainer).find('.lbf-autocomplete-suggestion').each(function (i) {
                            $(this).wrapInner('<a href="//sosu.qidian.com/searchresult.aspx?keyword=' + encodeURIComponent(data[i].value) + '" target="_blank" data-aid="qd_A13"></a>');
                        });
                    } else {
                        // 无结果隐藏
                        $('#s-box').click();
            ...
            function searchJump(btn, word) {
                btn.on('click', function () {
                    ...
                    var url = '//sosu.qidian.com/searchresult.aspx?keyword=' + searchVal;
            ...
            $('#s-box').on('keydown', function (evt) {
                if (evt.keyCode == 13) {
                    ...
                    // 事件触发超链接的方案，window.open是千万不能用的！
                var url = '//' +  g_data.envType + 'search.qidian.com/?kw=' + encodeURIComponent($(this).val());
                    var el = document.createElement("a");
                    document.body.appendChild(el);
                    el.href = url;
                    el.target = '_blank';
